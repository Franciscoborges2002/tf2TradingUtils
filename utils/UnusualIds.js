/**
 * TF2 Unusual particle effect id <-> name mappings, covering hats,
 * taunts and weapons, current through the latest update in the source
 * repo below.
 *
 * Sourced from https://github.com/Franciscoborges2002/tf2-unusual-ids
 * (all_simple.json / all_simple_reversed.json) — see that repo for
 * wiki links per effect and the hats/taunts/weapons breakdown.
 *
 * KNOWN LIMITATION: team-colored effects (two ids historically sharing
 * one base name, e.g. "Aces High (RED)"/"Aces High (BLU)") are
 * currently collapsed to a single id per name in the source repo itself
 * — only one of the two ids is reachable here until that repo
...
/** Effect id -> name — the reverse of UnusualIds above. */
